...
class PackageGeneratorService
{
    ...
    public function generatePackages(
        ...
        while (count($packages) < $this->maxPackages) {
            mt_srand((int) (microtime(true) * 1000000));  // Random seed

            $remainingBudget = $budget;
            $newPackage = $this->generatePackage(
                $availableProducts, $essentialItems, $remainingBudget, $userPreferences
            );
    ...
    private function generatePackage(
        ...
        shuffle($shuffledEssentials);  // Randomize essentials

        foreach ($shuffledEssentials as $essential) {
            $matchingProducts = [];

            foreach ($availableProducts as $product) {
                if ($product->name === $essential && 
